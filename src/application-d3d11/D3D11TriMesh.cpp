...
const D3D11_INPUT_ELEMENT_DESC MeshVertex::layout[layoutElementCount] =
{
...
void D3D11TriMesh::load(GraphicsDevice &g, const Vector<MeshVertex> &vertices, const Vector<UINT> &indices)
{
...
void D3D11TriMesh::initVB(GraphicsDevice &g)
{
	...
	bufferDesc.ByteWidth = sizeof( MeshVertex ) * (UINT)m_vertices.size();
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = 0;

	D3D11_SUBRESOURCE_DATA data;
	ZeroMemory( &data, sizeof(data) );
	data.pSysMem = m_vertices.ptr();
