...
template <class FloatType> inline FloatType Quaternion<FloatType>::angleDeg() const { return ( angleRad() * ( 360.0 / ml::math::PI ) ); }
...
template <class FloatType> Quaternion<FloatType> Quaternion<FloatType>::slerp( const Quaternion<FloatType>& q2, FloatType t ) const {
	...
	if ( ( 1.0 + CosOmega ) > delta ) {
		...
		// slerp
		scale0 = sin( ( (FloatType)1.0 - t ) * (FloatType)ml::math::PI / (FloatType)2.0 );
		scale1 = sin( t * (FloatType)ml::math::PI / (FloatType)2.0 );
