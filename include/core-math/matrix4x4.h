...
protected:
	...
	/*! used to calculate the complete 4x4 determinant */
	FloatType det3x3(unsigned int i0, unsigned int i1, unsigned int i2, unsigned int j0, unsigned int j1, unsigned int j2) const {
		return
...
__forceinline std::ostream& operator<<(std::ostream& s, const Matrix4x4<FloatType>& m)
{ 
	return (
		s << 
...
//! reads from a stream
template <class FloatType> 
__forceinline std::istream& operator>>(std::istream& s, const Matrix4x4<FloatType>& m)
{ 
	return (
		s >> 
