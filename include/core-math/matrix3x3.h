...
public:
	...
	bool computeEigenvaluesAndEigenvectorsNR(point3d<FloatType>& lambda, Matrix3x3<FloatType>& eigenvectors) const {
		point3d<FloatType> ev0, ev1, ev2;
		bool success = computeEigenvaluesAndEigenvectorsNR(lambda[0],lambda[1],lambda[2], ev0, ev1, ev2);
		eigenvectors = Matrix3x3(ev0, ev1, ev2).getTranspose();
		return success;
	}

	//! computes eigenvalues and eigenvectors numerically (numerical recipies); Eigenvalues/vectors are left to the matrix
	bool computeEigenvaluesAndEigenvectorsNR(FloatType &lambda_0, FloatType &lambda_1, FloatType &lambda_2, point3d<FloatType>& ev0, point3d<FloatType>& ev1, point3d<FloatType>& ev2) const {
