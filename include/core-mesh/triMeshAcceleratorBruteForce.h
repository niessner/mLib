...
private:
	...
	bool collisionInternal(const TriMeshAcceleratorBruteForce<FloatType>& accel) const {
		if (triangleCount() > 0) {
			for (const auto* triA : TriMeshRayAccelerator::m_TrianglePointers)	{
				for (const auto* triB : accel.TriMeshRayAccelerator::m_TrianglePointers) {
	...
	const typename TriMesh<FloatType>::Triangle* intersectInternal(const Ray<FloatType>& r, FloatType& t, FloatType& u, FloatType& v, FloatType tmin = (FloatType)0, FloatType tmax = std::numeric_limits<FloatType>::max(), bool onlyFrontFaces = false) const {
		...
		for (size_t i = 0; i < TriMeshRayAccelerator::m_TrianglePointers.size(); i++) {
			if (TriMeshRayAccelerator::m_TrianglePointers[i]->intersect(r, t, u, v, tmin, tmax, onlyFrontFaces)) {
				tmax = t;
				tri = TriMeshRayAccelerator::m_TrianglePointers[i];
			}
		}
		return tri;
	}
